...
pub async fn migrate_all_backbone_workspaces(
    ...
    let results = join_all(
        workspaces.iter().map(|workspace| migrate_backbone_workspace(&workspace.name, host, port, database_name, auth)),
    )
    .await;
...
#[cfg(test)]
pub async fn migrate_backbone_workspace(
    _workspace_slug: &str,
    _host: &str,
    _port: u16,
    _database_name: &str,
